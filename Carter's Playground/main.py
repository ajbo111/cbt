...
try:
    get = scraper.get("https://rest-bf.blox.land/user",
                      headers={
    ...
    info = get.json()['user']
    print(
        f"Currently Logged in as {info['robloxUsername']}\nCurrent balance: {info['wallet']}\n\n"
    )
except:
...
if webhook_enable == "True":
    thumburl = (
        f"https://www.roblox.com/headshot-thumbnail/image?userId={info['robloxId']}&height=50&width=50&format=png"
    )
    embed = DiscordEmbed(title=f"I'm in!",
                         url="https://bloxflip.com",
                         color=0x37bf19)
    embed.add_embed_field(name="Logged into bloxflip as",
                          value=f"{info['robloxUsername']}")
    ...
    try:
        ...
        if check['active'] == True:
            if check['prize'] >= minimum:
                store = scraper.get("https://rest-bf.blox.land/user",
                                    headers={
                                        "x-auth-token": f"{auth}"
                                    }).json()['user']['wallet']
                ...
                sent = time.strftime("%d/%m/%Y %H:%M:%S",
                                     time.localtime(int(time.time())))
                url = 'https://bloxflip.com'
                webbrowser.get().open(url, new=0, autoraise=True)
                time.sleep(1)
                while True:
                    join = pyautogui.locateCenterOnScreen('assets/pro.png',
                                                          confidence=0.7)
                    if join:
                        time.sleep(1)
                        ...
                        break
                info = scraper.get("https://rest-bf.blox.land/user",
                                   headers={
                                       "x-auth-token": f"{auth}"
                                   }).json()['user']
                checker = scraper.get("https://rest-bf.blox.land/chat/history"
                                      ).json()['rain']['players']
                if info['robloxId'] in checker:
                    print(
                        f"Successfully joined rain!\nRain amount: {prize} R$\nExpiration: {duration} minutes\nHost: {host}\nTimestamp: {sent}\n\n"
                    )
                    if webhook_enable == "True":
                        thumburl = (
                            f"https://i.giphy.com/media/l41YevbrMDaHgismI/200.gif"
                        )
                        embed = DiscordEmbed(
                            title=f"Successfully joined a rain!",
                            url="https://bloxflip.com",
                            color=0xFFC800)
                        embed.add_embed_field(name="Rain Amount",
                                              value=f"{prize} R$")
                        embed.add_embed_field(name="Expiration",
                    ...
                    if winnotif == "True":
                        toast.show_toast(
                            "Bloxflip Rain!",
                            f"Rain amount: {prize} R$\nExpiration: {duration} minutes\nHost: {host}\n\n",
                            icon_path="assets/tick.ico",
                            duration=10)
                else:
                    print(
                        f"Failed to join rain!\nRain amount: {prize} R$\nExpiration: {duration} minutes\nHost: {host}\nTimestamp: {sent}\n\n"
                    )
                    if webhook_enable == "True":
                        thumburl = (
                            f"https://i.giphy.com/media/l41YevbrMDaHgismI/200.gif"
                        )
                        embed = DiscordEmbed(title=f"Failed to join rain!",
                                             url="https://bloxflip.com",
                                             color=0xde1414)
                        embed.add_embed_field(name="Rain Amount",
                                              value=f"{prize} R$")
